...
public class SpecController {
     ...
     */
    @ApiOperation(value = "根据分类ID查询规格列表", notes = "根据分类ID查询规格列表方法详情", tags = {"SpecController"})
    @PostMapping(value = "/findSpecListByCategoryId")
    public Result<List<Spec>> findSpecListByCategoryId(@RequestBody(required = false) @ApiParam(name = "categoryId", value = "传入JSON数据", required = false) Integer categoryId) {
        //调用SpecService实现条件查询Spec
        List<Spec> list = specService.findSpecListByCategoryId(categoryId);
        return new Result<List<Spec>>(true, StatusCode.OK, "查询成功", list);
    }
