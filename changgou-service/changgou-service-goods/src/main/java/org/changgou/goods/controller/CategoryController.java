...
public class CategoryController {
    ...
    /***
     * 根据Category的parentId条件查询
     * @param parentId 父ID
     * @return Result<List < Category>>
     */
    @PostMapping(value = "/findListByParentId")
    @ResponseBody
    public Result<List<Category>> findListByParentId(String parentId) {
        //调用CategoryService实现条件查询Category
        List<Category> list = categoryService.findByParentId(Integer.parseInt(parentId));
        return new Result<List<Category>>(true, StatusCode.OK, "查询成功", list);
    }
